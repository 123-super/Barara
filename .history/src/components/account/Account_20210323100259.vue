      ...
      <el-form ref="form" :model="form" label-width="100px">
        ...
        <div>
          ...
          >
            ...
            <el-table-column prop="number" label="数量" width="240">
            </el-table-column>
            <el-table-column prop="city" label="实付款" width="240">
              <template v-slot="scope"
                ><span>{{ scope.row.price * scope.row.number }}</span></template
              >
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    account() {
      ...
      console.log
      let { orderNum, address, postNum, receiverName, tel, uid, status, createDate } = [status, uid, ...this.form]
      let orderitem = [pid, oid, uid, number]
      addOrder()
