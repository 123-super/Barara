...
import { delCurrentOrder, getMyOrder, getOrderUnPay, getOrdersUnDeliver, updLeftStatus } from "../../api/order"
...
export default {
  ...
  methods: {
    ...
    updLeftStatus(oid, ostatus) {
      let { id, status } = { oid, ostatus }
      if (ostatus == "待发货" && ostatus == "待发货") {
        updLeftStatus({ id, status }).then((res) => {
