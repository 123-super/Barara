            ...
            <div class="order" v-for="item in allOrders" :key="item.id">
              ...
              <div class="orderContent">
                ...
                <div class="status col-lg-1">
                  <el-button type="primary" @click="updLeftStatus(item, '状态')"
                    >{{ item.status }}
                  </el-button>
                </div>
              </div>
  ...
  methods: {
    ...
    getOrdersUnDeliver() {
      getOrdersUnDeliver().then((res) => {
        this.unDeliverOrder = res.data.data
      })
    },
    //待收货的所有订单
    getOrdersUnReceive() {
      getOrdersUnReceive().then((res) => {
        this.unReceiveOrder = res.data.data
      })
    },
    //待评价的所有订单
    getOrdersUnComment() {
      getOrdersUnComment().then((res) => {
        this.unCommentOrder = res.data.data
      })
    },
    //跳转到pay页面
    toPay(temp) {
