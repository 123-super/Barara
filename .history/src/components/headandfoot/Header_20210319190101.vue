...
export default {
  ...
  computed: {
    getTotalPrice() {
      return this.shopcarts.reduce((prev, good) => prev + good.count * good.price, 0)
    },
    getTotalNumber() {
      var totalnumber = 0
      this.multipleSelection.forEach((item) => {
        return totalnumber += item.number
        // console.log(item.number + "_____")
      })
      return totalnumber
    },
    // getPrice(x, y) {
  ...
  created() {
    ...
    let c = this.shopcarts.reduce((prev, good) => prev + good.count * good.pice, 0)
    console.log(c)
    // console.log(this.$store.state.shopcart[0].price)
